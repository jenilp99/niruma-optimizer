...
function initializeDefaults() {
    ...
    // v1.73: Force-sync formulas from code defaults for known hardware items.
    // localStorage preserves user-edited rates but formulas come from code (supplier registry).
    try {
        let hwMigrated = false;
        for (const [series, defaultItems] of Object.entries(defaultHardware)) {
            if (!Array.isArray(defaultItems) || !Array.isArray(hardwareMaster[series])) continue;
            defaultItems.forEach(def => {
                if (!def || !def.hardware || !def.formula) return;
                const saved = hardwareMaster[series].find(h => h && h.hardware === def.hardware);
                if (saved && saved.formula !== def.formula) {
                    console.log(`🔧 Updating ${series} / ${def.hardware} formula: "${saved.formula}" → "${def.formula}"`);
                    saved.formula = def.formula;
                    hwMigrated = true;
                }
            });
        }
        if (hwMigrated) {
            if (typeof autoSaveHardwareMaster === 'function') autoSaveHardwareMaster();
            console.log('🔧 Hardware formulas synced from code defaults');
        }
...
function getDoorHardwareList() {
    ...
    DOOR_HARDWARE_DEFAULTS.forEach(seed => {
        ...
        if (!existing) {
            ...
            if (existing.formula !== seed.formula) {
                existing.formula = seed.formula;
                changed = true;
            }
        }
    });
...
window.addEventListener('load', function () {
    ...
    // v1.98: one-time migration — set ACP 3mm partition rate to ₹95/sqft (user request).
    // Supersedes the v1.50 ₹90 migration. ratesConfig is already merged from
    // localStorage in initializeDefaults() by now. New flag so it re-applies once
    // for users still on the old ₹90.
    try {
        if (!localStorage.getItem('acp3mm95Migration')) {
            if (!ratesConfig.partitionRates) ratesConfig.partitionRates = {};
            ratesConfig.partitionRates['ACP_3mm'] = 95;
            localStorage.setItem('ratesConfig', JSON.stringify(ratesConfig));
            localStorage.setItem('acp3mm95Migration', '1');
            if (typeof renderPartitionRatesList === 'function') renderPartitionRatesList();
            console.log('🟧 Migrated ACP 3mm partition rate to ₹95/sqft');
        }
    ...
    try {
        if (!localStorage.getItem('doorHwRatesV52b')) {
            const list = getDoorHardwareList();
            const updates = {
                'Door Hinge':     { rate: 52,   variants: { '5"×1.25"': 76, '4"×1.25"': 52 } },
                'Floor Spring':   { rate: 3422, variants: { '90 kg': 2800, '100 kg': 3100, '120 kg': 3422 }, defaultVariant: '120 kg' },
                'Door Handle':    { rate: 502,  variants: { 'American Handle': 50, 'H-Handle': 502, 'D-Handle': 502 }, defaultVariant: 'H-Handle' },
                ...
                'Magnet':         { rate: 107,  variants: { 'No. 2 Magnet': 14, 'Ball Magnet': 107 }, defaultVariant: 'Ball Magnet' }
            };
            list.forEach(item => {
            ...
            if (typeof autoSaveHardwareMaster === 'function') autoSaveHardwareMaster();
            localStorage.setItem('doorHwRatesV52b', '1');
            console.log('🔧 Migrated door hardware rates to v1.52b market prices');
        }
